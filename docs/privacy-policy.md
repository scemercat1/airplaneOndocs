...
- To provide moderation functionality (`/warn`, `/mute`, `/ban`, etc.)
- To track bot usage for logs (if enabled by server owner)
- For improving the bot in future updates (only anonymous stats, if opted-in)

> **No data is shared with other servers or sold.**
...
- Redistribution of your self-hosted version is **not allowed** under the license
...
- Self-hosted version: Security depends on your server configuration
- Official bot version: Token and sensitive data are never shared
...
For the latest version, always check this documentation.
